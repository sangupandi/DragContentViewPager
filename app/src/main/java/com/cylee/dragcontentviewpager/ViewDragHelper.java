...
public class ViewDragHelper {
  ...
  public static abstract class Callback {
    ...
    public int getEdgeLeft(ViewDragHelper helper) {
      return helper.getEdgeSize();
    }

    public int getEdgeRight(ViewDragHelper helper) {
      return helper.getEdgeSize();
    }

    public int getEdgeBottom(ViewDragHelper helper) {
      return helper.getEdgeSize();
    }

    public int getEdgeTop(ViewDragHelper helper) {
      return helper.getEdgeSize();
    }
  }
  ...
  /**
   * Apps should use ViewDragHelper.create() to getInstance a new instance.
  ...
  public boolean smoothSlideViewTo(View child, int finalLeft, int finalTop, int duration) {
    ...
    boolean continueSliding = forceSettleCapturedViewAt(finalLeft, finalTop, 0, 0, duration);
    if (!continueSliding && mDragState == STATE_IDLE && mCapturedView != null) {
      // If we're in an IDLE state to begin with and aren't moving anywhere, we
      // end up having a non-null capturedView with an IDLE dragState
      mCapturedView = null;
    ...
    if (v instanceof ViewGroup) {
      ...
      for (int i = count - 1; i >= 0; i--) {
        ...
        if (x + scrollX >= child.getLeft() && x + scrollX < child.getRight() &&
            ...
            canScroll(child, checkV, dx, dy, x + scrollX - child.getLeft(),
    ...
    switch (action) {
      ...
      case MotionEvent.ACTION_MOVE: {
        ...
        for (int i = 0; i < pointerCount; i++) {
          ...
          if (pastSlop && !canScroll(toCapture, true, (int)dx, (int)dy, (int)x, (int)y) && tryCaptureViewForDrag(toCapture, pointerId)) {
            break;
          }
        }
        saveLastMotion(ev);
  ...
  private int getEdgesTouched(int x, int y) {
    ...
    mEdgeLeft = mCallback.getEdgeLeft(this);
    mEdgeRight = mCallback.getEdgeRight(this);
    mEdgeTop = mCallback.getEdgeTop(this);
    mEdgeBottom = mCallback.getEdgeBottom(this);

    if (x < mParentView.getLeft() + mEdgeLeft) result |= EDGE_LEFT;
    if (y < mParentView.getTop() + mEdgeTop) result |= EDGE_TOP;
    if (x > mParentView.getRight() - mEdgeRight) result |= EDGE_RIGHT;
    if (y > mParentView.getBottom() - mEdgeBottom) result |= EDGE_BOTTOM;
